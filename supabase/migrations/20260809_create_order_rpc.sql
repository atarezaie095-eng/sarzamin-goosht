-- Replace the earlier client-priced overload so it cannot remain callable.
drop function if exists public.create_order(text, text, text, text, integer, jsonb);

-- Atomic anonymous order creation with database-authoritative product pricing.
...
begin
  ...
  loop
    ...
    select price, coalesce(discount, 0)
      into v_product_price, v_product_discount
...
-- Anonymous callers must use the RPC. No table read or mutation privilege is
-- granted here; existing authenticated/admin privileges are left unchanged.
revoke insert, select, update, delete on table public.orders from anon;
revoke insert, select, update, delete on table public.order_items from anon;
